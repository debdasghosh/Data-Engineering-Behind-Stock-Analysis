...
setwd("D:\\MSIS\\sem4\\DE\\project\\Datasets\\StockTwits\\")

sysProps <- read.properties("D:\\R\\StockDWBI\\config\\config.properties")
last_updated_record_id <- sysProps$last_updated_record_id

url <- "https://api.stocktwits.com/api/2/streams/symbol/AMZN.json?max="
...
repeat{
  ...
  tryCatch( { 
      ...
      if(last_updated_record_id>=starting_index){
        ...
        #write.properties(file="D:\\R\\StockDWBI\\config\\config.properties",properties=list(sysProps))
        quit(status=0)
        break
      }
