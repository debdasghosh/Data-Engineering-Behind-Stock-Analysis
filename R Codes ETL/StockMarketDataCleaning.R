...
setwd("D:\\R\\StockDWBI\\config")
sysProps <- read.properties("config.properties")
sysProps
setwd("D:\\MSIS\\sem4\\DE\\project\\Datasets\\Stocks")
...
generateYStocksDataCompanyWise <- function(symbol_code){
  ...
  if(sysProps$is_live_stock_enabled=='0')
    y_stocks_data<- y_stocks_data[(y_stocks_data$Date >= "2017-01-01" & y_stocks_data$Date <= "2021-04-08"),]
...
for(stock_symbol in stocks_files) {
  ...
  if(!file.exists(file_name) || file.size(file_name) ==0 || sysProps$is_live_stock_enabled=='1') {
    ...
    startDate = "1483228800" # 2017-01-01 00:00:00
    appURL <- paste0("https://query1.finance.yahoo.com/v7/finance/download/",stock_symbol,"?period1=",startDate,"&period2=",endDate,"&interval=1d&events=history&crumb=NIzM/9c35H4")
